...
def find_grade_sections(pdf_path, state_abbr, title):
    ...
    grade_patterns = {
        "K": ["Kindergarten", "Grade K"],
        "1": ["First Grade", "Grade 1", "1st Grade", "GRADE 1"],
        "2": ["Second Grade", "Grade 2", "2nd Grade", "GRADE 2"],
        "3": ["Third Grade", "Grade 3", "3rd Grade", "GRADE 3"],
        "4": ["Fourth Grade", "Grade 4", "4th Grade", "GRADE 4"],
        "5": ["Fifth Grade", "Grade 5", "5th Grade", "GRADE 5"],
        "6": ["Sixth Grade", "Grade 6", "6th Grade", "GRADE 6"],
        "7": ["Seventh Grade", "Grade 7", "7th Grade", "GRADE 7"],
        "8": ["Eighth Grade", "Grade 8", "8th Grade", "GRADE 8"],
    }
        ...
        try:
            ...
            for grade, patterns in grade_patterns.items():
                ...
                for pattern in patterns:
                    if pattern in text:
                        # Verify it's a heading (not just mention in text)
                        lines = text.split('\n')
...
def main():
    states = [
        ("wy_standards_2021.pdf", "WY", "2021 Wyoming Science Standards"),
        ("wa_standards.pdf", "WA", "Washington State K-12 Science Learning Standards"),
    ]

    all_patches = {}

    for pdf_file, state_abbr, title in states:
        ...
        grade_sections = find_grade_sections(pdf_file, state_abbr, title)
        ...
        if grade_sections:
            ...
            print(f"\n[OK] {state_abbr}: {len(grade_sections)} grade sections")
        else:
            print(f"\n[SKIP] {state_abbr}: No sections found")

    # Save patch
    if all_patches:
        output = Path("patches/final_states.json")
